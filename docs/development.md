...
Workspace invitations (Phase 10A): see [invitations.md](./invitations.md). Local/test default is `EMAIL_PROVIDER=console` (logs the accept URL, including the raw token). Production must use `EMAIL_PROVIDER=smtp`. Do not expect the API to return invitation tokens.
