...
export function AuthChrome() {
  ...
  return (
    <div className="flex items-center justify-end gap-2" data-testid="auth-chrome">
      <div
        role="group"
        aria-label={t('shell.language')}
        className="inline-flex rounded-full border border-border/80 bg-card/75 p-1 shadow-sm"
      >
      ...
      <Tooltip>
        ...
        <TooltipContent>{t(`shell.${themeLabelKey(currentTheme)}`)}</TooltipContent>
      </Tooltip>
    </div>
  );
}
