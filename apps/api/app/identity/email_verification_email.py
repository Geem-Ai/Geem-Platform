"""Email-verification copy (plain text + HTML). User-controlled fields are escaped in HTML."""
...
def render_email_verification_email(
    *,
    ...
    subject = "Verify your Geem email"
    text_body = _text_body(verify_url=verify_url, expires=expires, email=safe_email)
    html_body = _html_body(verify_url=verify_url, expires=expires, email=safe_email)
    return EmailVerificationContent(subject=subject, text_body=text_body, html_body=html_body)
...
def _text_body(*, verify_url: str, expires: str, email: str) -> str:
    account = f"Account: {email}\n" if email else ""
    account_ar = f"الحساب: {email}\n" if email else ""
    return (
        "Verify your Geem email\n\n"
        f"{account}"
        f"This link expires: {expires}\n\n"
        f"Verify your email:\n{verify_url}\n\n"
        "If you did not create a Geem account, you can ignore this email. "
        "Do not share this link.\n\n"
        "—\n"
        "Geem\n\n"
        "تأكيد بريدك على جِيم\n\n"
        f"{account_ar}"
        f"ينتهي الرابط: {expires}\n\n"
        f"أكد بريدك الإلكتروني:\n{verify_url}\n\n"
        "إذا لم تنشئ حساباً على جِيم يمكنك تجاهل هذه الرسالة. لا تشارك هذا الرابط.\n\n"
        f"Website: {GEEM_WEBSITE_URL}\n"
        f"Support: {GEEM_SUPPORT_URL}\n"
    )


def _html_body(*, verify_url: str, expires: str, email: str) -> str:
    safe_expires = escape(expires)
    safe_email = escape(email)
    href = quoteattr(verify_url)
          ...
          <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e7eb;">
            ...
            <tr>
              <td style="padding:28px;">
                <h1 style="margin:0 0 12px;color:{BRAND};font-size:22px;font-family:Arial,sans-serif;">Verify your email</h1>
                <p style="margin:0 0 16px;color:#52606d;font-size:15px;line-height:1.5;font-family:Arial,sans-serif;">
                  Confirm this email address to finish creating your Geem account.
                </p>
                ...
                <p style="margin:20px 0 0;color:#8a94a6;font-size:13px;line-height:1.5;font-family:Arial,sans-serif;">
                  If you did not create this account, you can ignore this email. Do not share this link.
                </p>
                <hr style="border:none;border-top:1px solid #e4e7eb;margin:28px 0;" />
                <h2 dir="rtl" style="margin:0 0 12px;color:{BRAND};font-size:20px;font-family:Arial,sans-serif;text-align:right;">تأكيد البريد الإلكتروني</h2>
                <p dir="rtl" style="margin:0 0 16px;color:#52606d;font-size:15px;line-height:1.5;font-family:Arial,sans-serif;text-align:right;">
                  أكّد هذا البريد لإكمال إنشاء حسابك على جِيم.
                </p>
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 20px;">
                  {email_row_ar}
                  <tr>
